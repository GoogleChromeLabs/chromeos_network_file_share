...
// TODO(XXX): This came from NaCl SDK. Should this be moved to third_party?
...
var common = (function() {
  ...
  function runTests(moduleEl) {
    ...
    common.tester.addAsyncTest('loaded', function(test) { test.pass(); });
  ...
  function createNaClModule(name, tool, path, width, height, attrs) {
    ...
    moduleEl.setAttribute('PS_VERBOSITY', '0');
    moduleEl.setAttribute('src', path + '/' + name + '.nmf');

    // Make NaCl printf got to the console
    //moduleEl.setAttribute('PS_STDOUT', '/dev/console1');
    ...
    if (isTest) {
      var loadNaClTest = function() {
        injectScript('nacltest.js', function() { runTests(moduleEl); });
      };
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('In common.js - DOMContentLoaded');
  ...
  if (body.dataset) {
    ...
    if (loadFunction) {
      ...
      var tc = toolchains.indexOf(searchVars.tc) !== -1 ? searchVars.tc :
                                                          toolchains[0];
